...
- 名言点击创建的翻译任务会按“名言警句翻译+年月日时分秒”写入历史标题。
- 支持多选导出 EPUB（向导参数、章节排序）。
- 新增 EPUB 闭环翻译：导入 EPUB -> 拆章节 HTML -> 批量翻译写历史 -> 自动导出 `_已翻译.epub`。
- EPUB 闭环翻译改为逐章节即时写入历史，单章失败不会导致之前章节历史丢失。
- EPUB 章节标题与来源标识分离：历史标题保留章节真实标题，来源文件信息单独展示。
...
- EPUB 导出支持“仅译文”内容模式，适配竖排阅读场景（减少双语并排带来的阅读干扰）。
- EPUB 文件名新增目标语言后缀（如 `_Japanese.epub`），便于多语言版本区分。
- 日文竖排样式已优化顶部起排（含 `text-align:start` 与版面边距修正）。
- EPUB 闭环增加试用限制：未开启“我已诚信付费”时默认只处理前三章，开启后可完整导出。
- EPUB 闭环向导与导出向导均新增可视化进度条，按总章节/打包进度显示进度百分比。

5. 翻译引擎与偏好
- 引擎管理表格化：新增、编辑、启停、软删除、默认引擎切换。
- 无引擎时引导配置，无默认引擎时要求显式选择。
- 记录保留引擎快照信息，软删除后历史可回显“已删除”状态。
- 用户偏好支持默认导出目录、默认 EPUB 作者，以及“我已诚信付费”完整导出开关。
