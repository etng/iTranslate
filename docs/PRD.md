...
- 历史记录：
  ...
  - 存储采用 SQLite，数据库位于用户配置目录下的 `iTranslate/history.db`。
  - EPUB 场景下标题与来源信息分离：标题使用章节真实标题，文件来源独立记录。
- 翻译引擎管理（表格化）：
  - 新增、编辑、启停、软删除、设为默认。
...
- EPUB 导出：
  ...
  - 支持手动排序（上移/下移）以处理同名章节或混合来源记录。
  - 支持日文竖排阅读模式（章节竖排 + `spine` 右开翻页方向）。
  - 支持“仅译文”导出模式，并在文件名追加目标语言后缀，便于阅读与版本区分。
  - 修复 iBooks 兼容性（实体处理）。
  - 导出成功后 toast 提示。
- EPUB 闭环翻译：
  - 支持导入 `.epub`，自动提取章节 HTML/XHTML 并按翻译流程批量处理。
  - 每个章节结果写入历史，标题规则：`{epub文件名} ⟫ {html文件名}`。
  - 章节翻译完成即写入历史（增量落库），避免批量任务中途失败导致历史全丢。
  - 批量翻译完成后自动导出 `{epub文件名}_已翻译.epub`。
  - 默认试用模式仅处理前三章；用户在偏好中启用“我已诚信付费”后可完整导出全部章节。
  - 闭环流程需展示可视化进度条（按章节总量）。
- 用户偏好：
  - 默认 EPUB 导出目录。
  - 默认 EPUB 作者。
...
- 翻译或闭环任务失败时，必须提供前台可见提示（如 `alert`），不能仅写日志。
